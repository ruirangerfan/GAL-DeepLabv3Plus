...
if __name__ == '__main__':
    ...
    writer = SummaryWriter(os.path.join(train_opt.checkpoints_dir, train_opt.name))
    ...
    for epoch in range(train_opt.epoch_count, train_opt.nepoch + 1):
        ...
        valid_loss_iter = []
        epoch_iter = 0
        conf_mat = np.zeros((valid_dataset.dataset.num_labels, valid_dataset.dataset.num_labels), dtype=np.float32)
        with torch.no_grad():
